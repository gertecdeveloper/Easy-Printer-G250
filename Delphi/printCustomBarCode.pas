...
uses
  Classes, SysUtils, synaser, blcksock, openDoor, CloseDoor, sendBinBuffer, Vcl.Dialogs;
...
procedure printCustomBarCodeFunction(height:integer; width:integer; hri:integer; codeType:string; text: string; printer : TBlockSerial) overload;
procedure printCustomBarCodeFunction(height:integer; width:integer; hri:integer; codeType:string; text: string; printer : TTCPBlockSocket) overload;
...
procedure printCustomBarCodeFunction(height:integer; width:integer; hri: integer;
  codeType:string; text: string; printer : TBlockSerial);
begin
  sendBinBufferFunction(TEncoding.ASCII.GetBytes(printCodeBarFactory(height, width, hri, codeType, text)), printer);
end;
...
begin
  ...
  if((Length(text)<codeBar.minimumLength) or (Length(text)>codeBar.maximoLength)) then
    raise Exception.Create('Quantidade de caracteres incorreta. Para o código '+codeBar.name
          + ' são necessários no mínimo '+IntToStr(codeBar.minimumLength)+' e no máximo '+IntToStr(codeBar.maximoLength)
          +' caracteres.');
  ...
  If(codeBar.codeToPrint <= 6) then
    ...
    buffer := height_concat + width_concat + hri_concat + PRINT + chr(Length(text)) + text;
