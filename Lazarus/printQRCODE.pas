...
procedure printQRCODE(size:integer; corretionLevel:integer; text: string; printer : TBlockSerial);
procedure printQRCODE(text: string;printer : TBlockSerial);

procedure printQRCODE(size:integer; corretionLevel:integer; text: string; printer: TPrinter);
procedure printQRCODE(text: string;printer: TPrinter);

procedure printQRCODE(size:integer; corretionLevel:integer; text: string; printer : TTCPBlockSocket);
procedure printQRCODE(text: string;printer : TTCPBlockSocket);

function printQRCODEFactory(size:integer; corretionLevel:integer; text: string):string;

implementation

procedure printQRCODE(size: integer; corretionLevel: integer;
  text: string; printer : TBlockSerial);
begin
  sendBinBuffer.sendBinBuffer(TEncoding.GetEncoding(860).ASCII.GetBytes(printQRCODEFactory(size, corretionLevel, text)), printer);
end;

procedure printQRCODE(text: string; printer : TBlockSerial);
begin
  printQRCODE(5, 48, text, printer);
end;
